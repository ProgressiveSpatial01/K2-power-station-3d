...
// a single continuous run — via a 12d reference screenshot, same as
// how the design-linework fire water case was confirmed. Add to this
...
function buildLineFeaturesFrom12d(records, layerKind) {
  ...
  const features = records.flatMap((r) => {
    const [firstE, firstN] = r.centrelinePoints[0] ?? [];
    const coordKey = firstE != null ? `${Math.round(firstE)},${Math.round(firstN)}` : "";
    const confirmedContinuous = CONFIRMED_CONTINUOUS_SERVICE_RECORDS.has(`${r.model}|${r.name}|${coordKey}`);
    const segments =
